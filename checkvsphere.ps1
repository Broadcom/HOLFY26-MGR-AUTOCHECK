...
If ( $vcPresent ) { 
	...
	ForEach ( $s in $global:DefaultVIServers ) {
		...
		$version = $s.Version
		$build = $s.Build
		If ( $build -eq "20426518" ) {
			Write-Logs "FAIL" $name $function "$name is running the RTM vCenter $version build $build which is not allowed."
			Continue
		}
		If ( ( $vcVersion[$version] -eq $build ) -Or ( $vcVersion["${version}VCF"] -eq $build ) ) {
...
Foreach ($h in $allhosts) {
	...
	If ( ( $esxVersion[$version] -eq $build ) -Or ( $esxVersion["${version}VCF"] -eq $build ) ) {
